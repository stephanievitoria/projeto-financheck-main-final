...
export function TransactionModal({ isOpen, onClose, onSubmit, initialTransaction }: TransactionModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name || !finalCategory || isNaN(numericAmount)) return;

    const today = new Date();
    const monthNames = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'];
    const dateStr = `${today.getDate().toString().padStart(2, '0')} ${monthNames[today.getMonth()]}`;
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-[520px] bg-white/70 backdrop-blur-3xl rounded-[32px] p-10 shadow-[0_20px_60px_rgba(91,143,232,0.15),0_0_0_1px_rgba(255,255,255,0.5)_inset] border-2 border-white/70 transition-all duration-300 ease-out">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                required
              />
              <p className="mt-2 text-[#7BA8F5] text-xs" style={{ fontFamily: 'Inter, sans-serif', fontWeight: 400 }}>
                Use valores negativos para despesas (ex: -150.00)
              </p>
            </div>
            ...
            >
              ...
              <span className="relative">{loading ? 'Salvando...' : 'Confirmar Transação'}</span>
            </button>
          </form>
        </div>
      </div>
